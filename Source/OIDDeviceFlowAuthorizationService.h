/*! @file OIDDeviceFlowAuthorizationService.h
...
@class OIDAuthState;
@class OIDDeviceFlowAuthorizationRequest;
@class OIDDeviceFlowAuthorizationResponse;
@class OIDDeviceFlowServiceConfiguration;
...
/*! @brief The block that is called when the TV authorization has initialized.
    @param response The authorization response, or nil if there was an error. Display
        @c OIDDeviceFlowAuthorizationResponse.userCode and @c OIDDeviceFlowAuthorizationResponse.verificationURL to
        the user so they can action the request.
    @param error The error if an error occurred.
 */
typedef void (^OIDDeviceFlowAuthorizationInitialization)
    (OIDDeviceFlowAuthorizationResponse *_Nullable response,
    NSError *_Nullable error);

/*! @brief The block that is called when the TV authorization has completed.
    @param authState The @c OIDAuthState which you can use to authorize
        API calls, or nil if there was an error.
    @param error The error if an error occurred.
 */
typedef void (^OIDDeviceFlowAuthorizationCompletion)(OIDAuthState *_Nullable authState,
                                                     NSError *_Nullable error);
...
/*! @brief Block returned when authorization is initialized to that will cancel the pending
        ...
        concluded.
 */
typedef void (^OIDDeviceFlowAuthorizationCancelBlock)(void);

/*! @brief Performs authorization flows designed for TVs and other limited input devices.
 */
@interface OIDDeviceFlowAuthorizationService : NSObject
 ...
 */
+ (OIDDeviceFlowAuthorizationCancelBlock)authorizeTVRequest:(OIDDeviceFlowAuthorizationRequest *)request
                                     initializaiton:(OIDDeviceFlowAuthorizationInitialization)initialization
                                         completion:(OIDDeviceFlowAuthorizationCompletion)completion;
